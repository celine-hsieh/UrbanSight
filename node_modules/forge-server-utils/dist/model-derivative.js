...
class ModelDerivativeClient extends common_1.ForgeClient {
     ...
     * @throws Error when the request fails, for example, due to insufficient rights.
     */
    async getViewableTree(urn, guid, force, objectId) {
        const url = this.getBaseUrl(this.region === common_1.Region.EMEA ? `regions/eu/designdata/${urn}/metadata/${guid}` : `designdata/${urn}/metadata/${guid}`);
        if (force)
     ...
     * @throws Error when the request fails, for example, due to insufficient rights.
     */
    async getViewableTreeStream(urn, guid, force, objectId) {
        const url = this.getBaseUrl(this.region === common_1.Region.EMEA ? `regions/eu/designdata/${urn}/metadata/${guid}` : `designdata/${urn}/metadata/${guid}`);
        if (force)
            url.searchParams.append('forceget', 'true');
        if (objectId)
     ...
     * @throws Error when the request fails, for example, due to insufficient rights.
     */
    async getViewableProperties(urn, guid, force, objectId) {
        const url = this.getBaseUrl(this.region === common_1.Region.EMEA ? `regions/eu/designdata/${urn}/metadata/${guid}/properties` : `designdata/${urn}/metadata/${guid}/properties`);
        if (force)
            url.searchParams.append('forceget', 'true');
        if (objectId)
            url.searchParams.append('objectid', objectId.toString());
     ...
     * @throws Error when the request fails, for example, due to insufficient rights.
     */
    async getViewablePropertiesStream(urn, guid, force, objectId) {
        const url = this.getBaseUrl(this.region === common_1.Region.EMEA ? `regions/eu/designdata/${urn}/metadata/${guid}/properties` : `designdata/${urn}/metadata/${guid}/properties`);
        if (force)
            url.searchParams.append('forceget', 'true');
        if (objectId)
